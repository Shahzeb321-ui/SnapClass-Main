...
def get_teacher_subjects(teacher_id):
    response = supabase.table('subjects').select("*, subjects_student(count), attendence_log(timestamps)").eq("teacher_id", teacher_id).execute()
...
def create_attendance(logs):
    res = supabase.table('attendence_log').insert(logs).execute()
    return res.data

def get_attendance_for_teacher(teacher_id):
    res= supabase.table('attendence_log').select('*,subjects!inner(*)').eq('subjects.teacher_id',teacher_id).execute()
    return res.data
