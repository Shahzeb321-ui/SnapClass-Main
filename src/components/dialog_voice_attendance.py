...
from src.pipelines.voice_pipleline import process_bulk_audio
import pandas as pd

from datetime import datetime

@st.dialog("Voice Attendance Dialog")
def voice_attendance_dialog(selected_subject_id):
    ...
    if st.button('Analyze Audio', width='stretch', type='primary'):
        with st.spinner('Prcessing Audio data'):
            enrolled_res = supabase.table('subjects_student').select("*, students(*)").eq('subject_id',selected_subject_id ).execute()
